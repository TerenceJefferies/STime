...
describe('TimeFromBuilder', () => {
    it('Should return an instance 3 months in the past', () => {
        const builder = new TimeFromBuilder(3, undefined, undefined, new Time(new Date()));
        const time = builder.months().inPast();
        const date = new Date();
        date.setMonth(date.getMonth() - 3);
        expect(time.getMonth()).toEqual(date.getMonth() + 1);
    });

    it('Should return an instance 3 days in the future', () => {
        const builder = new TimeFromBuilder(3, undefined, undefined, new Time(new Date()));
        const time = builder.days().inFuture();
        const date = new Date();
